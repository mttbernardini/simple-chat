...
<head>
<meta charset="UTF-8" />
<title>Chat 3G - Cronologia messaggi</title>
<link rel="icon" type="image/x-icon" href="/2g/favicon.ico" />
...
<div id="back"><a href="index.php">Torna indietro</a></div>
...
foreach ($db->msg as $msg) {
	if ($msg->type=="int")
		...
		elseif ((date('Gi', $current) >= date('Gi', $last) + 2) || (is_int(date("i", $current)/$timestampEach) && !is_int(date("i", $last)/$timestampEach) )) {
		  echo '<p class="time">' . date('H:i', $current) . "</p>\n";
		  $lastAut="";
		}
